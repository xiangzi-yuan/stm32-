...
int main(void)
{
	...
	Delay_ms(1000);
	USART_MUSIC(Music_Cmd_Appoint, 1);
	while(1)
	{   
		if(RTC_Get()==0)
		{
...
//		OLED_ShowNum(10,week,ClockTime[0],3,16,1);//显示TIM时钟，调试用
		...
		if(clock[1]==ClockTime[1])
		{
			ClockResponse(1);
		}
		if(clock[2]==ClockTime[2])
		{
			ClockResponse(2);
		}
		if(clock[3]==ClockTime[3])
		{
			ClockResponse(3);
		}
		//睡眠提醒
		...
		if(testflag == 1){
			...
			if(DHT_Buffer[2]>40){
				OLED_Clear();
				OLED_ShowString(2*size,size,"temp!!",16,1);
				OLED_Refresh();
				Delay_ms(1000);
				OLED_Clear();
			}
			if(DHT_Buffer[0]>60){
				OLED_Clear();
				OLED_ShowString(3*size,size,"humi!!",16,1);
				OLED_Refresh();
				Delay_ms(1000);
				OLED_Clear();
			}
